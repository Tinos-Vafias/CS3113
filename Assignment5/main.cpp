...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("ASSIGNMENT 5",
...
void process_input()
{
    ...
    if (g_current_scene->number_of_lives != 0) {
...
void update()
{
    ...
    if (g_current_scene == g_levelA && g_current_scene->m_state.player->get_position().y < -10.0f && g_current_scene->m_state.player->get_position().x > 20.0f) switch_to_scene(g_levelB,g_total_lives);
